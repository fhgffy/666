...
/*
 * 需求背景：
 *  PAD 与综显同时进行全局规划时，可能导致 DTMS / 综显 / PAD 规划状态不一致。
 *
 * 修改策略：
 *  1) 规划过程中的信息仅发送给“调用方”(规划发起端)；
 *  2) 另一端在调用“生成全局规划结果(航线生成)”时，若调用方未发布航线，则反馈“XX自主规划中”；
 *  3) 点击航线发布时，同步【任务分解/分配结果】与【当前无人机航线】给 PAD 与综显。
 */

typedef enum
{
	DTMS_PLAN_SRC_NONE = 0,
	DTMS_PLAN_SRC_DPU  = 1,   // 综显
...
// 全局规划互斥：当 owner 存在且与 requester 不同，则认为忙
...
/* 仅向指定端发送 CCC_DPU_data_0（方案/航线状态），mask: DTMS_SEND_TO_DPU / DTMS_SEND_TO_PAD */
...
/* 反馈“XX自主规划中”给 requester（用于互斥拒绝） */
static void dtms_send_global_plan_busy_error(DTMS_PLAN_SRC_E requester)
{
	sprintf(CCC_DPU_data_0.failreason, "%s自主规划中", dtms_plan_src_name(g_global_plan_owner));

	// 航线生成失败(3)，方案类型按全局规划(1)回
...
/* 点击航线发布：同步任务分配结果 + 当前无人机航线（PAD + 综显） */
static void dtms_sync_global_plan_result(unsigned int plan)
{
	...
	// 1) 同步任务分配结果：复用 send_blk_ccc_ofp_019_special()，需临时切换 blk_ccc_ofp_019
...
void stage_send(){
	...
	// 优先接收当前发起端的数据，避免另一端刷消息造成“调用方”消息被饿死
	...
	if(enRetCode == 0)
	{
		// 互斥：另一端正在自主规划且未发布航线时，本端禁止生成全局规划结果
		...
		// 绑定全局规划发起端（后续规划过程信息只回传给该端）
		g_global_plan_owner = req_src;

		scheme_generation_state(1,2, 0, 1); // 航线生成中（只回传给调用方）
		//判断无人机是否在线
		int normal_cnt = 0;
		for(int i = 0 ; i < 4 ; i ++)
		{
		...
		if(CCC_DPU_data_3.drone_number > normal_cnt)
		{
			...
			scheme_generation_state(1,2,0,3);// 生成失败（只回传给调用方）
			memset(&CCC_DPU_data_0.failreason,0,sizeof(CCC_DPU_data_0.failreason));
			return;
		}
		//判断输入合法性
		...
		if(blk_ofp_ccc_038.Plan_ID <= 3)
		{
			...
			send_blk_ccc_ofp_018(conn_id,plan,task); // 发送有人机通航点
...
void rev_zhanfa_plan(){
	...
	if(zhanfa_result_receive_flag == 1)
	{
		...
		if(enRetCode == 0)
		{
			...
			// 全局规划：规划过程信息仅发送给调用方；航线发布同步阶段发给两端
...
void receive_zhanfa_hl(){
	...
	if(enRetCode == 0)
	{
		...
		// 加入航路点
		if(temp.airway_point_start_num == 0){  // 第一包40点
			//取基本数据
...
void init_blk_ccc_ofp_021()
{
	...
	for(int i = 0; i < 4 ; i ++)
	{
		...
		for(int j = 0; j < 8 ; j ++)
		{
			//取任务参数
			/*任务平台子任务ID号*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].id
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].id;
			/*子任务任务类型*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].type
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].type;
			/*子任务任务点/区域类型  0=N/A;1=任务点;2=线;3=区域;*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].point_or_area
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].point_or_area;
			/*任务区/点/线/目标编号*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].point_or_area_id
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].point_or_area_id;
			/*子任务完成时间有效位*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].finish_time_valid
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].finish_time_valid;
			/*子任务任务高度有效位*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].finish_height_valid
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].finish_height_valid;
			/*子任务完成时间*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].finish_time
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].finish_time;
			/*子任务完成高度*/
			blk_ccc_ofp_021.plan_info[i].task_sequence_informations[j].finish_height
			= blk_ccc_ofp_019.formation_synergy_mission_programs[i+1].task_sequence_informations[j].finish_height;
		}
	}
}
...
void init_blk_ccc_ofp_026()
{
	...
	if(s6_bd_flag == 1)
	{
		...
		else if(s7_hx_flag == 0)
		{
			...
			{
				//具备条件
				if(s9_manned_lead.conditon1 && s9_manned_lead.conditon2 && s9_manned_lead.conditon3 && s9_manned_lead.conditon4
						 && s9_manned_lead.conditon7 && s9_manned_lead.conditon8)
				{
					//编队-按航线飞-编队保持-具备领航
...
//有人机航路信息发送
void send_buoy_soanr_route_information(unsigned int plan,int task){ // i 为无人机自任务
...
void send_blk_ccc_ofp_024_single(unsigned int plan,unsigned int single_index)
{
	...
	for(unsigned char j = 0 ;j < blk_ccc_ofp_024_cunchu[plan][single_index].individual_drone_routing_programs.planning_informations.total_packet;j++)
	{
		...
		memcpy(&temp.individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[0]
			,&blk_ccc_ofp_024_cunchu[plan][single_index].individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[j*25]
			 ,sizeof(planning_information_waypoint_information)*25);
		//包序号赋值
...
void send_blk_ccc_ofp_024(unsigned int plan)
{
	...
	for(int i = 0 ; i < 4 ; i ++)
	{
		...
		for(unsigned char j = 0 ;j < blk_ccc_ofp_024_cunchu[plan][i].individual_drone_routing_programs.planning_informations.total_packet;j++)
		{
			...
			//取航路点
			memcpy(&temp.individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[0]
			                                                                                                                ,&blk_ccc_ofp_024_cunchu[plan][i].individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[j*25]
			                                                                                                                                                                                                                                                     ,sizeof(planning_information_waypoint_information)*25);
			//包序号赋值
			temp.individual_drone_routing_programs.planning_informations.packet_id = j;
...
void formulate_moduel(){
	...
	if(enRetCode == 0)
	{
		// 非调用方的任务区操作直接丢弃（避免另一端同步规划时污染当前规划流程）
		...
		else
		{
			...
			{   //检查反潜
				...
				scheme_generation_state(1,1,0,0);// 返回方案编辑状态到调用方
				formulate_flag = 0;
			}
			//增加任务区
...
void route_change(int i)
{
	...
	if(i == 2 && plan != 0)
	{
		...
		for(unsigned char j = 0 ;j < blk_ccc_ofp_024_cunchu[plan][id].individual_drone_routing_programs.planning_informations.total_packet ; j++)
		{
			...
			//取航路点
			memcpy(&temp.individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[0]
			   ,&blk_ccc_ofp_024_cunchu[plan][id].individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[j*25]
			   ,sizeof(planning_information_waypoint_information)*25);
			//包序号赋值
			temp.individual_drone_routing_programs.planning_informations.packet_id = j;
			// 综显发送
...
void edit_uav_route(int i)
{
	...
	if(i == 2 && uav_id != 0)
	{
		...
		for(unsigned char j = 0 ;j < blk_ccc_ofp_024_single[id].individual_drone_routing_programs.planning_informations.total_packet ; j++)
		{
			...
			//取航路点
			memcpy(&temp.individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[0]
			                                                                                                                ,&blk_ccc_ofp_024_single[id].individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[j*25]
			                                                                                                                                                                                                                                                ,sizeof(planning_information_waypoint_information)*25);
			//包序号赋值
			temp.individual_drone_routing_programs.planning_informations.packet_id = j;
			// 综显发送
	...
	if(enRetCode == 0)
	{
		if(DPU_CCC_data_12.mission_object_type == 2 || DPU_CCC_data_12.mission_object_type == 3)//目标/光标
		{
			...
			else
			{
				...
				{
					sprintf(CCC_DPU_data_0.failreason,"未检测到有效无人机返航航线");
					scheme_generation_state(0,2,0,3);// 生成失败
					memset(&CCC_DPU_data_0.failreason,0,sizeof CCC_DPU_data_0.failreason);
					return;
				}
			}
...
void init_single_mission_target_zhanfa(){
	...
	if(DPU_CCC_data_12.mission_object_type == 1){  //(任务区信息)
		//索引任务区
		int index = -1;
		int packge = -1;
		for(int i = 0 ;i < 8 ; i++)
	...
	}else if(DPU_CCC_data_12.mission_object_type == 2){//(目标信息)光标选点
	...
	else if(DPU_CCC_data_12.mission_object_type == 3 || DPU_CCC_data_12.mission_object_type == 4){//(光标选点/航路点)
		memset(&(blk_dtms_ctas_002.region_infos),0,sizeof(blk_dtms_ctas_002.region_infos));
		// 任务点信息
		blk_dtms_ctas_002.region_infos[0].task_point_num = 1;
		//编队飞行
...
void init_single_uav_zhanfa(){
	...
	blk_dtms_ctas_002.zhanshu_sel = 1;   // 战术战法选择(废弃不使用)
	if(DPU_CCC_data_11.mission_object_type == 1){  //(任务区信息)
		blk_dtms_ctas_002.task_reg_num = 1;  // 任务区数量
...
//	avoidLineCrashFabuProc();
	...
	//进入发送
		...
		//判断是否发送完一架无人机的所有航路点
		if (frame_count > 4 * uav_send[send_index].waypoints_number + 1) { //机场点+一般航迹点+倒数第二个计算点+最后一个汇合点，所以是+3，最后加1是确保最后一个点发送四包
...
void uav_simulation() {
	...
	if (enRetCode == 0)
	{
		// 全局规划航线发布：同步任务分配结果 + 当前无人机航线给 PAD 与综显
		// 说明：routeType 以现网约定为准；此处按全局规划(=1)处理
...
void send_uav_hl(unsigned int uav_id,unsigned char order){
	...
	case 0x38:{   // 航点（绝对）装订指令
		...
		}else if(track_point_count < uav_send[send_index].waypoints_number - 3 && uav_send[send_index].first_flag  == 1){ // 航点计数
...
//			order_data_frames.track_point_jd_order.lon = 124.24993 /lon_scale;//塔哈机场写死 124.24993
		...
		else if(track_point_count >= uav_send[send_index].waypoints_number -2){ // 发送最后一个点 机舰点
...
// 处理赋值有效位
short set_data_valid(short data,int judg_data,int b){  // data 进行比特位赋值的数据  judg_data 判断有效的数据  b 赋值的比特位
	if(judg_data > 0){
...
void init_drone_information_Azhen(int uav_index){
	...
	} else if (DPU_CCC_data_14.video_params_setting_feedbacks.ir_pwr_ctrl == 0x02) {   //红外下电
		// 指令类型: 0X20 控制状态
		CCC_UAV_Azhens[uav_index].guangdiankongzhizhilings.zhiling_class = 0X20;
		// 光电控制命令 红外电源关
...
//		}
	...
	if (g_image_main_channel == 0x00) { //电视
	...
	} else if (g_image_main_channel == 0x01) {  //红外
	...
	= setBit(CCC_DPU_data_22[uav_index].photoelectric_state_feedbacks.target_param_significant_bit, 0, 0);// 目标1参数有效位
	if(Target1status==1) {
		CCC_DPU_data_22[uav_index].photoelectric_state_feedbacks.target_param_significant_bit
		= setBit(CCC_DPU_data_22[uav_index].photoelectric_state_feedbacks.target_param_significant_bit, 0, 1);// 目标1参数有效位
	...
	CCC_DPU_data_22[uav_index].photoelectric_state_feedbacks.target_param_significant_bit
	= setBit(CCC_DPU_data_22[uav_index].photoelectric_state_feedbacks.target_param_significant_bit, 8, 0);// 目标2参数有效位
	if(Target2status==1) {
		CCC_DPU_data_22[uav_index].photoelectric_state_feedbacks.target_param_significant_bit
		= setBit(CCC_DPU_data_22[uav_index].photoelectric_state_feedbacks.target_param_significant_bit, 8, 1);// 目标2参数有效位
...
void avoidLineCrashJudgeSingle(int drone_index) {
	...
	for (int i = 0; i < man_airway_point_num+1; i++) { // 加1是加上有人机当前位置
		...
		for(int j =0; j<uav_waypoints_number+1; j++){ // 加1是加上
...
void avoid_doulbe_uav(int drone_index,ConflictResult tmp)
{
	...
	if(g_lineCrashState[0].hasConflict == 1 && g_lineCrashState[1].hasConflict == 1)
	{
		...
		memcpy(&blk_ccc_ofp_024_cunchu[plan][avoid_index].individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[1],
				...
				sizeof(planning_information_waypoint_information) * 75);
		//经纬度
		blk_ccc_ofp_024_cunchu[plan][avoid_index].individual_drone_routing_programs.planning_informations.planning_information_waypoint_informations[0].latitude
...
void payload_task()
{
	...
	for (unsigned int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < 8; j++) {
			if (task_param[j].type_id
					== CCC_DPU_data_6[plan].formation_synergy_mission_programs[i + 1].task_sequence_informations[blk_ofp_ccc_039.stage_id].type) {
				//找到索引退出
				uav_send[i].task_index = j;
				break;
			}
